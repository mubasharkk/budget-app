...
- **Dashboard** logic lives in `app/Services/Dashboard/DashboardService.php`; the controller exposes JSON endpoints (`/dashboard/stats`, `/dashboard/chart/data`, etc.) consumed by React. Note this service uses raw `DB::` query-builder joins for aggregation rather than Eloquent.
